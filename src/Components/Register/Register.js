...
function Register() {
  ...
  const formik = useFormik({
    ...
    onSubmit:values=>{
      ...
      formData.append("profilefile", proileFile);
      ...
      axios.post('https://max-bid.herokuapp.com/createUsers',formData).then((response)=>{
       // console.log(response);
        ...
        { 
          alert ("Profile picture/ idproof not selected");
        }
  ...
  return (
    <div>
        ...
        <div className="input1">
<label className='idmsg'>UPLOAD PROFILE PICTURE</label>
        <input className='form-control form-control-lg file11' type="file" name="profilefile" accept="image/*"onChange={(event) => {
              setProfileFile(event.target.files[0]);
              setProfileFileName(event.target.files[0].name);

            }} />
                      {formik.touched.phone && formik.errors.filepic ? <span style={{color:'red'}}>{formik.errors.filepic}</span> : null}
